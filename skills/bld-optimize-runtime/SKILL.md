---
name: bld-optimize
description: Run the third-party code-optimizer MCP servers (jcodemunch, context-mode) on-demand for a single burst of code search, then shut them down — no persistent connection. Use when the user says /bld-optimize, "hard optimize", "token-optimize this search", or is about to do a big code sprint across many files and wants compact, cheap code exploration.
---
...
```bash
cd <project-root>
python .claude/skills/bld-optimize-sprint/run.py jcodemunch <<'EOF'
[
...
```bash
python .claude/skills/bld-optimize-sprint/run.py shadcn <<'EOF'
