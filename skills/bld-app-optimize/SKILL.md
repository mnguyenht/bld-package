---
name: bld-app-optimize
description: Audit a whole web app with Google Lighthouse — performance, accessibility, best practices, SEO — then triage the findings and fix the ones that are real. Use when the user says /bld-app-optimize, "run Lighthouse", "check my PageSpeed", "why is my site slow", "audit the live site", "get my scores up", or before shipping something that strangers will load on mobile. Measures the shipped app end to end; /bld-react-optimize reads the source instead.
---

# bld-app-optimize — measure the shipped app, then fix what the numbers point at

`/bld-react-optimize` reads your **source** and tells you what looks wrong.
...
|---|---|---|---|
| `/bld-react-optimize` | source files | hooks bugs, dead code, unsafe patterns | anything that only appears once bundled and served |
| **`/bld-app-optimize`** | the running page | LCP, layout shift, blocking JS, contrast, crawlability | logic errors, anything below the fold, anything behind a login |

Run this one **after** the app is deployed or at least built for production, and
after `/bld-react-optimize` has cleared the obvious source-level defects.
...
Same discipline as `/bld-react-optimize`: **findings are hypotheses.**
...
| SEO < 100 | usually meta or crawlability → that is `/bld-seo-indexing`, not this skill |
...
- **Chasing SEO points here.** A low SEO score means missing meta or blocked
  crawling. That is `/bld-seo-indexing`, and it also will not make Google index
  you faster.
