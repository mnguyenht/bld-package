---
name: bld-handoff
description: Write a handoff.md snapshot of the current session (goal, state, active files, changes, failed attempts, next steps) so work can continue in a fresh session before context rot sets in. Use when the user says /bld-handoff, "hand off", "write a handoff", "save state and start fresh", or wants to switch sessions without losing context.
---
