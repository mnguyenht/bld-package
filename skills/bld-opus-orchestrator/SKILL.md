---
name: bld-opus-orchestrator
description: The bld-fable-orchestrator plan → execute → judge loop with Opus in the boss seat. Use when the user says /bld-opus-orchestrator, or wants the orchestrator loop run on Opus rather than Fable. Claude never writes the code.
model: opus
---

# bld-opus-orchestrator — the same loop, Opus in the boss seat

**Read `.claude/skills/bld-fable-orchestrator/SKILL.md` and run it exactly.** Every
