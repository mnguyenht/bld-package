---
name: bld-planning
description: Decide what to build before anything gets built — interview the idea, pressure-test the assumption it rests on, cut it to a shippable v1, and write the durable planning artifacts (PRODUCT.md, planning.md, progress.md). Use when the user says /bld-planning, "help me plan this", "I have an idea", "what should I build", "plan the next feature", or arrives with a vague idea and no shape. Ends by handing off to /bld-init. Does not write app code.
---
...
| **`/bld-planning`** | *Should this exist, for whom, paid for how, and what is the smallest version?* | Yes. Three files on disk. |
...
Under ~30 lines, written by `/bld-init` at scaffold time. If the app already
...
4. **The next command.** `/bld-init` for a new app; plan mode or straight to work
