---
name: bld-fable-orchestrator
description: Boss mode over Claude subagents — turn a brain-dump into a plan → execute → judge loop. Claude scopes, specs, fans out bld-executor workers in parallel, judges their reports as a skeptic, and re-specs until the work meets the bar. Use when the user says /bld-fable-orchestrator, drops a messy multi-item to-do list, or wants several independent pieces built at once. Claude never writes the code.
model: fable
---

# bld-fable-orchestrator — Claude is the head, the executors are the hands

The user's brain-dump comes after `/bld-fable-orchestrator`. **Claude never writes the
...
One spec per workstream, using the template in `.claude/skills/bld-agents/SKILL.md`
...
- `/bld-agents` has been invoked this session (its instructions are in context), or
...
- Workspace deploy discipline holds: this loop is **local only**. No commit, no push, no
  Vercel unless the user says ship — then `/bld-deploy`.
...
- On Opus, `/bld-opus-orchestrator` runs this same loop with Opus in the boss seat;
